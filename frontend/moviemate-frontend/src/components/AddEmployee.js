...
export default function AddEmployee( props ) {
    ...
    async function addEmployee( name, surname, email, insurance ) {
        ...
        let url = process.env.REACT_APP_URL + "employee?name=" + name + "&surname=" + surname + "&email=" + email +
            "&password=" + createPassword() + "&socialInsuranceNumber=" + insurance + "&adminId=" + props.userData.id
    ...
    return <Dialog open={ true } onClose={ () => props.onClose() } fullWidth maxWidth={ "md" }>
        <Grid container style={ {
            ...
            <Grid container style={ { display: "flex", justifyContent: "center", width: "60rem" } }>
                <LoginRegisterTextField value={ insurance } setValue={ ( val ) => setInsurance( val ) }
                                        label={ "Insurance" }/>
            </Grid>
